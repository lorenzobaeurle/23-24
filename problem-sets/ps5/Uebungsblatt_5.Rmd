...
# Direktes Berechnen und Ausgeben des 99% Konfidenzintervalls für die Regression y~x
confint(lm(y ~ x), level = 0.99)

print(confint(lm(y ~ x), level = 0.99))
...
library(stargazer)
stargazer(reg1, reg2, reg3, type = "text", title = "Regressionsergebnisse", align = TRUE)
...
# Laden des Datensatzes jtrain3 aus dem wooldridge Paket
library(wooldridge)
data(jtrain3)

# Zählen der Personen, die an der Weiterbildungsmaßnahme teilgenommen haben
n_train <- sum(jtrain3$train == 1)

# Anzeigen der Anzahl
print(paste("Anzahl der Personen, die an der Weiterbildung teilgenommen haben:", n_train))

# Zeige den Wert von n_train
n_train
...
jtrain3 %>% 
  group_by(unem75) %>%  
  summarize(
    Average_Training_Participation = mean(train, na.rm = TRUE), 
    Unemployment_Rate_1978 = mean(unem78, na.rm = TRUE)  
  )
...
# Sicherstellen, dass der Datensatz verfügbar ist
data(jtrain3)

# Durchführen der Regression mit 'unem78' auf 'train'
reg1 <- lm(unem78 ~ train, data = jtrain3)

# Durchführen der Regression mit 'unem78' auf 'train' und 'unem75'
reg2 <- lm(unem78 ~ train + unem75, data = jtrain3)

# Anzeigen der Ergebnisse von reg1 und reg2 nebeneinander mit stargazer
stargazer(reg1, reg2, type = "text", title = "Vergleich der Regressionsmodelle")
...
# Erstellen von jtrain3_la für Langzeitarbeitslose
jtrain3_la <- jtrain3 %>% 
  filter(unem75 == 1)

# Erstellen von jtrain3_ka für Kurzzeitarbeitslose oder Nicht-Arbeitslose
jtrain3_ka <- jtrain3 %>% 
  filter(unem75 == 0)
...
# Laden des Datensatzes, falls noch nicht geladen
data(jtrain3)

# Erstellen der Subsets für Langzeitarbeitslose und Nicht-Langzeitarbeitslose
